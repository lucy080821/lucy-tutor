...
- **CLASSES** (Lớp Học) — Quản lý lớp học (tạo/sửa/xóa), xem danh sách học sinh. Nút xóa lớp (`ClassCard` → `onDelete`) hiện khi hover, xác nhận qua SweetAlert2; xóa cascade toàn bộ bài học/đề thi/tài liệu/điểm danh/học phí của lớp đó
  - **Cách tính học phí chọn lúc tạo/sửa lớp** (`Classroom.feeType`, "PER_LESSON" mặc định hoặc "MONTHLY"), 2 chế độ chạy song song, không đổi lẫn nhau giữa các lớp: **Theo Buổi** (`feePerLesson` × số buổi PRESENT trong tháng — vắng không tính tiền, hành vi gốc không đổi) hoặc **Theo Tháng** (`feePerMonth` quy đổi theo tỉ lệ buổi học thực tế — xem chi tiết công thức bên dưới, không còn là mức phí cố định bất kể đi học bao nhiêu buổi như trước). Backend gom logic vào 1 hàm dùng chung `calcTuitionAmount(classroom, presentCount, month, year)` trong [backend/src/routes/attendance.routes.js](backend/src/routes/attendance.routes.js), áp dụng cho cả 3 báo cáo (`/report/:classroomId`, `/report/teacher/:teacherId`, `/my-tuition/:userId`) — tránh lệch công thức giữa các endpoint. `TuitionInvoice` (hóa đơn PDF) và badge chế độ tính phí ở đầu bảng "Báo Cáo Học Phí" tự đổi nhãn/mô tả theo `feeType` của từng lớp
    - **Công thức "Theo Tháng"**: `calcTuitionAmount` gọi `getStandardLessonsInMonth(classroom, month, year)` — duyệt từng ngày trong tháng đang xét, đếm số ngày trùng với `Classroom.scheduleDays` (mảng thứ trong tuần, vd `[1,4]` = Thứ 2 & Thứ 5) để ra **số buổi học chuẩn thật của đúng tháng đó** (không phải ước lượng cố định "4 tuần × N buổi/tuần" — tháng có 5 Thứ 2 thay vì 4 sẽ tự nhiên ra nhiều buổi chuẩn hơn, khớp đúng yêu cầu "cộng số lẻ nếu có"). Học phí = `feePerMonth ÷ số buổi chuẩn trong tháng × số buổi PRESENT thực tế` (làm tròn `Math.round`). Nếu lớp chưa đặt `scheduleDays` (hoặc rỗng/không parse được), `getStandardLessonsInMonth` trả `null` → fallback về hành vi cũ: thu trọn gói `feePerMonth` bất kể số buổi (không có lịch cố định thì không có cơ sở quy đổi theo buổi)
    - Các endpoint báo cáo trả kèm `classroom.standardLessons` (chỉ có giá trị với lớp MONTHLY đã đặt lịch) để frontend hiển thị đúng "X/Y buổi" và đơn giá/buổi thay vì nhãn "1 tháng" cũ — xem badge chế độ tính phí, cột "Số Buổi Học" ở bảng Báo Cáo Học Phí, và dòng mô tả trong `TuitionInvoice`
...
- **Highlight đề bài + Ghi chú nháp lúc làm bài** — thuần client-side, không tính điểm, không ảnh hưởng chấm bài:
  - **Highlight**: bôi đen văn bản trong khối đề bài (`question.heading` + `question.content`, cả 2 đều là HTML từ ReactQuill) rồi bấm nút 🖍 nổi lên cạnh vùng chọn để tô vàng. Do nội dung là HTML chứ không phải text thuần, không dùng `Range.surroundContents()` trên cả range (sẽ throw ngay khi selection cắt qua nhiều thẻ) — thay vào đó đi qua từng text node giao với Range (`document.createTreeWalker` + `range.intersectsNode`) và bọc `<mark class="exam-user-highlight">` riêng cho từng đoạn. Bấm lại vào chỗ đã highlight để bỏ (unwrap `<mark>`). Vùng đề bài được gắn `select-text` để ghi đè `select-none` ở root (root vẫn `select-none` khi đang làm bài thật — chỉ mở lại đúng vùng cần bôi đen, không ảnh hưởng phần đáp án/điều hướng)
  - **Ghi chú**: nút "📝 Ghi Chú" bật/tắt 1 ô textarea theo từng câu (key = `question.id`), lưu **tạm ở `localStorage`** (`examNotes_{examId}_{userId}`) — không gửi lên server, không xem lại được sau khi rời trang/nộp bài (quyết định có chủ đích: đây là giấy nháp hỗ trợ tư duy, không phải 1 phần bài làm cần chấm hay lưu trữ)
  - Không xung đột với chống gian lận copy/paste đang có — thao tác bôi đen/ghi chú không kích hoạt sự kiện `copy`, chỉ có Ctrl+C/X/V thật mới bị chặn và log gian lận như cũ
